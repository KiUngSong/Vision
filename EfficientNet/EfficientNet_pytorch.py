...
# Can add any modified version's model name
VALID_MODELS = (
...
# Mobile Inverted Residual Bottleneck Block : Main Component of EfficientNet
...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Since global average pooling is used, agnostic to input sizes
    ...
    @classmethod
    # Create an efficientnet model according to name which can modify global_params 
    # by using override_params: e.g. 'width_coefficient', 'depth_coefficient', 'image_size', 'num_classes'
    ...
    @classmethod
    # Create efficientnet model according to name with pretrained weight loaded
    # Can modify override_params but not recommended for pretrained model
...
# Apply drop connection to given input tensor [B,C,W,H] with probability p
...
# Chooses static padding if you have specified an image size, and dynamic padding otherwis
...
class BlockDecoder(object):
    @staticmethod
    # Get a block through a string notation of arguments : e.g. r1_k3_s11_e1_i32_o16_se0.25_noskip
...
# with "override_params" to modify global_params
def get_model_params(model_name, override_params):
    ...
    if override_params:
        # ValueError will be raised if override_params has fields not included in global_params.
        global_params = global_params._replace(**override_params)
    return blocks_args, global_params


# Pretrained weight from https://github.com/lukemelas
url_map = {
...
def load_pretrained_weights(model, model_name, weights_path=None, load_fc=True):
    """weights_path (None or str): 
        str: path to pretrained weights file on the local
        None: use pretrained weights downloaded from the URL
    """
