...
from EfficientNet_utils_pytorch import (
    round_filters,
    round_repeats,
    drop_connect,
...
class AugTrainer(Trainer):
    ...
    def mixup_criterion(self, pred, y_a, y_b, lam):
        return lam * self.loss_ftn(pred, y_a) + (1 - lam) * self.loss_ftn(pred, y_b)
    ...
    def train(self, n_epoch, verbose_freq=20, aug_rate=0.2):
        for i in tqdm(range(n_epoch)):
            self.model.train()
            train_loss = 0
            correct, total = 0, 0
            for _, (images, labels) in enumerate(self.train_loader):
                ...
                if random.random() < aug_rate:
                    ...
                    if random.uniform(0,1) > 0.5:
                        ...
                        bbx1, bby1, bbx2, bby2 = self.rand_bbox(images.size()[2], images.size()[3], lam)
                        ...
                        self.optim.zero_grad()
                        outputs = self.model(images)
                        loss = self.loss_ftn(outputs, labels_a) * lam + self.loss_ftn(outputs, labels_b) * (1. - lam)
                        loss.backward()
                        self.optim.step()
                    # MixUp
                    else:
